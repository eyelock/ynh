...
func reviewExistingArtifact(vendor, root, overview, path, artifactType string, skipConfirm bool) error {
	...
	updatePrompt := fmt.Sprintf(`You are updating an existing %s for an AI-powered development harness.
...
Output ONLY the complete updated file content, nothing else.`,
	...
	updated, llmErr := queryLLM(vendor, updatePrompt)
	if llmErr != nil {
...
func generateSingleArtifact(vendor, overview, prop, root string) (string, error) {
	signals := scanSignals(root)
...
Output ONLY the raw file content starting with ---.`, prop, overview, signalContext)
