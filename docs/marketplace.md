...
Copilot's plugin loader (`--plugin-dir`) reads the same `.claude-plugin/plugin.json` schema Claude Code uses, confirmed by hands-on testing. Its marketplace/index format, however, was only researched from documentation, not hand-verified against a real published marketplace — treat `ynd marketplace build`'s Copilot output as a best-effort starting point, not a confirmed-working artifact.
...
| `harnesses[].type` | Yes | `"harness"` or `"plugin"` |
| `harnesses[].source` | Yes | Local path or remote Git source (see below) |
| `harnesses[].description` | No | Overrides plugin.json description |
| `harnesses[].version` | No | Overrides plugin.json version |
| `harnesses[].path` | No | Monorepo subdirectory within a remote repo |

**Source formats:**

| Format | Example | Notes |
|--------|---------|-------|
| Relative path | `"./plugins/my-plugin"` | Resolved relative to marketplace.json |
| Absolute path | `"/home/ci/harnesses/reviewer"` | Used as-is |
| GitHub shorthand | `"github.com/org/repo"` | Cloned and cached; same shorthand as harness `git:` includes |
| HTTPS URL | `"https://github.com/org/repo.git"` | Any Git host |
| SSH URL | `"git@github.com:org/repo.git"` | Any Git host |

> **Note:** The `github.com/org/repo` shorthand works in both harness `git:` includes and `harnesses[].source`. However, the vendor-native `marketplace.json` format (inside `.claude-plugin/` or `.cursor-plugin/`) uses a different object syntax for remote plugins — that format is output generated by ynh, not authored directly.
