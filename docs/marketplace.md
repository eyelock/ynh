...
| `entries[].source` | Yes | Local path or remote Git source (see below) |
| `entries[].description` | No | Overrides plugin.json description |
| `entries[].version` | No | Overrides plugin.json version |
| `entries[].path` | No | Monorepo subdirectory within a remote repo |
...
> **Note:** The `github.com/org/repo` shorthand works in both harness `git:` includes and `entries[].source`. However, the vendor-native `marketplace.json` format (inside `.claude-plugin/` or `.cursor-plugin/`) uses a different object syntax for remote plugins — that format is output generated by ynh, not authored directly.
