...
Every structured response carries a `capabilities` field — `config.CapabilitiesVersion`, currently `0.7.0`. Schemas carry the same value as an `x-capabilities` annotation.
